			...
			<div class="row">
				...
				<div class="item-field col-md-6 col-sm-6 col-xs-12">
					<div class="item-field-inner">
						<textarea name="note" placeholder="<?php echo esc_html__('Order Notes', 'monalisa'); ?>"></textarea>
					</div>
				</div>
		...
		<div class="payment-button-field">
			<div class="payment-method">
				...
				?>
					...
					<label class="payment-item payment-deposit <?php (isset($booking_info_data['deposit_price']) && $booking_info_data['deposit_price']) ? '' : 'hidden'; ?>"><input type="radio" name="payment_method" value="deposit" <?php echo $contact_data['payment_method'] == 'full' ? 'deposit' : ''; ?>> <?php echo sprintf(wp_kses(__('Payment online deposit price <span class="booking-deposit-price">%s</span>', 'monalisa'), inwave_allow_tags('span')), iwBookingUtility::getMoneyFormated($booking_info_data['deposit_price'])); ?></label>
				<?php }else{ ?>
					<input type="hidden" name="payment_method" value="direct">
				<?php } ?>
			</div>
			<div class="submit-field">
				<button type="button" class="reservation-process-from pay-button iwb-button theme-bg"><?php echo $paypal ? __('Pay now', 'monalisa') : __('Contact now', ''); ?></button>
			</div>
		</div>
	</form>
</div>
